...
websites = [url.strip() for url in input("Websites to scrape (use ',' for multiple websites): ").split(',')]
...
def scrape_website(url):
    try:
        ...
        if response.status_code == 200:
            ...
            css_urls = re.findall(r'<link.*?href=["\'](.*?)["\']', response.text, flags=re.IGNORECASE)
            js_urls = re.findall(r'<script.*?src=["\'](.*?)["\']', response.text, flags=re.IGNORECASE)
            ...
            print(Fore.RED + f"Failed to scrape website {url}. Status code: {response.status_code}")
